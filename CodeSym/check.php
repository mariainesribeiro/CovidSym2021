<?php
   $connect = mysqli_connect('localhost', 'root','','COVIDSYM') or die('Error connecting to the server: '.mysqli_error($connect));
					...
					$dist = $_POST['newDist'];
					echo "<div class='w3-display-topmiddle w3-container w3-panel w3-green'> <h3>".$morada.$nome."</h3></div>";
	...
	$update_USERNAME = 'UPDATE UTILIZADORES as U SET U.USERNAME= REPLACE(U.USERNAME,"'.$_POST['old_username'].'","'. $user.'") WHERE ID = '.$_GET['id'];
	...
	echo "<div class='w3-display-topmiddle w3-container w3-panel w3-green'> <h3>A alteração foi registada com sucesso!</h3></div>";
