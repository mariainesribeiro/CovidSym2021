<?php
    			...
    			$select_admin = 'SELECT * FROM UTILIZADORES WHERE ID='.$_GET['id'];
...
<div class="w3-card-4 w3-container w3-display-topmiddle w3-theme-l5" style="width:70%; height:100%; overflow:scroll;">
	...
	<form name ="registo" method="POST" class="w3-container"  enctype="multipart/form-data" >
			...
			<br>
			<input type=hidden id ="old_username" name ="old_username" value="<?php echo $admin['USERNAME']; ?>">
			<label class="w3-text-theme"><strong>Username: </strong></label><input class=" w3-input w3-border-0"  onblur="checkNewUsername(this.value)" type="text" name="newUser" value="<?php echo $admin['USERNAME'];?>" disabled>
    		...
    		<label class="w3-text-theme"><strong>Nome completo: </strong></label> <input class=" w3-input w3-border-0" type="text" name="newName" value="<?php echo $admin['NOME'];?>" disabled>
		    <br>
		   	<div class="w3-row-padding">
 		    	<div class="w3-half">
 		    	<input type=hidden id ="old_email" name="old_email" value="<?php echo $admin['EMAIL']; ?>">

 		    	<label class="w3-text-theme"><strong>Email: </strong></label> <input class ="w3-input w3-border-0"  type="email" name="newEmail" value="<?php echo $admin['EMAIL'];?>"disabled >
 		    	</div>
 		    	<div class="w3-half">
 		    	<label class="w3-text-theme"><strong>Telem&oacute;vel: </strong></label> <input class ="w3-input w3-border-0" type="text" name="newTel" value="<?php echo $admin['TELEMOVEL'];?>" disabled>
 		    	</div>
 		    </div>
			<br>

			<label class="w3-text-theme"><strong>Morada: </strong></label> <input class=" w3-input w3-border-0" type="text" name="newAddress" value="<?php echo $admin['MORADA'];?>" disabled>
		    ...
		    <div class="w3-row-padding">
 		    	<div class="w3-half">
 		    		<label class="w3-text-theme"><strong>Distrito</strong></label>
 		    	      <select name="newDist" id ="newDist" class="w3-select"  onchange="localidadesSelecao(this.value)" selected ="<?php echo $admin['DISTRITO'];?>" disabled>
                         	<?php
 		    	...
 		    	<div class="w3-half">
  		    	      ...
  		    	      <select id ="localidade" name="newLocal" class="w3-select"  selected ="<?php echo $admin['LOCALIDADE'];?>" disabled>
								<?php
